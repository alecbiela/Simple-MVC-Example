...
// sets a dog in the DB
const setDog = (req, res) => {
  // check for bad data
  if (!req.body.fullname || !req.body.breed || !req.body.age) {
    return res.status(400).json({ error: 'fullname, breed, and beds are all required' });
  }
  ...
  // create new object of Dog Model
  const newDog = new Dog(dogData);
  const savePromise = newDog.save();

  savePromise.then(() => {
      // set lastadded to current add
    lastAddedDog = newDog;
      // return a success with the data
...
// same as above, but to search for dog
